    ...
    def display_schoolbook(self):
        ...
        if self.reserve:
            return (
                ...
                f"страниц: {self.count_page}, "
                f"Предмет: {self.item}, Класс: {self.classroom}, "
                f"{self.reservation()}"
            )
        return (
            f"Название: {self.title}, Автор: {self.author}, "
...
schoolbook_1 = SchoolBook("Алгебра", "Ерёменко",
                          120, "342-000", True, "Математика", 9)
print(schoolbook_1.display_schoolbook())
schoolbook_2 = SchoolBook(
    "История Беларуси", "Щёлков", 200, "534-012", False, "История", 8
)
